...
//     int reuse_op     = 1;
//     int ipv6_only_op = 0;
//     // When destroying this socket we want it to instantly free so we can
//     re-use
...
//     setsockopt(socket_fd, SOL_SOCKET, IPV6_V6ONLY, &ipv6_only_op,
//     sizeof(int));
...
//     int b_res = bind(socket_fd, (sockaddr *)addr.get(),
//     sizeof(sockaddr_in6));
...
volt::listener::listener(std::uint16_t hostport)
    : io_context(), sock(io_context),
      ipv4_acceptor(this->io_context, tcp::endpoint(tcp::v4(), this->port)),
      ipv6_acceptor(this->io_context, tcp::endpoint(tcp::v6(), this->port))
{
    this->port = hostport;

    ipv4_acceptor.async_accept(
        sock, boost::bind([=]() { this->accept_new_connection(); }));
    ipv6_acceptor.async_accept(
        sock, boost::bind([=]() { this->accept_new_connection(); }));

    this->thr = std::thread([=]() { this->io_context.run(); });

    this->listener_open = true;
